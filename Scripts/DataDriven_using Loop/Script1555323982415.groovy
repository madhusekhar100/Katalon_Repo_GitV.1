...
for (int i = 1; i <= findTestData('21cv/21cvtestdata').getRowNumbers(); i++) {
    WebUI.sendKeys(findTestObject('Object Repository/Page_CURA Healthcare Service/username'), findTestData('21cv/21cvtestdata').getValue(
            1, i))

    WebUI.waitForElementPresent(findTestObject('Object Repository/Page_CURA Healthcare Service/pswd'), 5)

    WebUI.sendKeys(findTestObject('Object Repository/Page_CURA Healthcare Service/pswd'), findTestData('21cv/21cvtestdata').getValue(
            1, i))
	WebUI.delay(3)
}
